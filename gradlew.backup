...
if "$cygwin" || "$msys" ; then
    ...
    ROOTDIRSRAW=$(find -L / -maxdepth 3 -type d -name java_modules 2>/dev/null | head -1)
    if [ -n "$ROOTDIRSRAW" ] ; then
        ROOTDIRS=$(cygpath --path --unix "$ROOTDIRSRAW")
    fi
    case $1 in
        -*)
            is_absolute_path=$(expr "$1" : '\(.*\)[/\\].*$' \| "$1")
            is_relative_path=$?
            if [ "$is_relative_path" = 0 ]; then
                args="$1"
            else
                args=$(cygpath --path --unix "$1")
            fi
            shift
            ;;
    esac
...
# Collect all arguments for the java command, following the shell word splitting rules
